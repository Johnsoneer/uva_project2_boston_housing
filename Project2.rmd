...
```
After generating the binary variable, we generated visualizations to see how the other variables interact. Specifically, we focused on the price variable. 
...
Our benchmark adjusted R-squared suggests that nearly 74% of the variance in our data is explained by our model. Our F-statistic is large and the relative p-value suggests the following result to a hypothesis test:
...
The reduced model looks much better since the p-values for both coefficients are close to zero.e The coefficient for rm is positive, which indicates that holding the average home value constant, increasing the average number of rooms increases the probability that the neighborhood is black. 
